...
def read(filespec, verbose=False):
    ...
    """
    shape, maxval = dims(filespec, verbose)
    with open(filespec, "rb") as f:
        f.readline()  # discard header line
        dtype_in = ">u2" if maxval > 255 else np.uint8
        dtype_out = np.uint16 if maxval > 255 else np.uint8
        pixels = np.fromfile(f, dtype_in)
        pixels = pixels.reshape(shape)
        pixels = pixels.astype(dtype_out)
